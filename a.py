...
if submitted:
    ...
    for i, c in enumerate(cluster_df1.drop(['Cluster'], axis=1).columns):
        if i == 0:
            continue  
        fig = px.histogram(cluster_df1, x=c, color="Cluster", marginal="box")
        st.plotly_chart(fig)
        print(f"Plotted histogram for column {c}")
    clust = loaded_model.predict(data)[0]
    print('Data Belongs to Cluster', clust)
    ...
    fig, axes = plt.subplots(1, len(cluster_df1.columns) - 1, figsize=(20, 3))

    for i, c in enumerate(cluster_df1.drop(['Cluster'], axis=1)):
        
        sns.histplot(data=cluster_df1, x=c, kde=True, ax=axes[i], color='skyblue')
        axes[i].set_title(c)
